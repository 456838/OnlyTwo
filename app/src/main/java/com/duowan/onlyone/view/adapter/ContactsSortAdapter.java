...
import com.duowan.onlyone.model.entity.contacts.SortModel;
...
public class ContactsSortAdapter extends BaseAdapter implements SectionIndexer {
	private final DecimalFormat MusicSizeFormat = new DecimalFormat("##0.0");
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			viewHolder = new ViewHolder();
				view = LayoutInflater.from(mContext).inflate(R.layout.adapter_item_contact, null);
		...
		viewHolder.tvTitle.setText(this.mList.get(position).musicName);
		viewHolder.tvSinger.setText(this.mList.get(position).musicSinger+" · " + MusicSizeFormat.format(this.mList.get(position).size / 1024.0 / 1024.0) + "M");
//		" · " + MusicSizeFormat.format(model.size / 1024.0 / 1024.0) + "M"
		return view;
